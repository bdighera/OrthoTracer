...
class treeOBjFileHandler():
    ...
    def fix_coding_direction(self, workingRecord, geneID):
        ...
        This function only applies to the parent protein, it will not modify any of the neighboring genes
        '''
        ...
        if workingRecord[0]['flip'] == False:
            for i in range(maxLength):
                try:
                    ...
                    return FixedRecord
        if workingRecord[0]['flip'] == True:
            for i in range(maxLength):
                try:
                    if i == parentProteinIndex:
                        location = {'img_start': start_gene_location[middleGeneIndex],
                                    'img_end': end_gene_location[middleGeneIndex]}
                        FixedRecord.append(dict(workingRecord[i], **location))
...
def lineMotifs(groups, switch):
    if switch == '-':
        return [[i[0], i[1], "[]", None, 2, "LightGrey", "LightGrey", None] for i in groups]
    else:
        return [[i[0], i[1], "[]", None, 2, "Black", "Black", None] for i in groups]