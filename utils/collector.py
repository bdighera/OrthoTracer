...
from utils.handler import IntronFileHandler

class SequenceCollector():

    def __init__(self, proteinRecord, linux, apikey):
    ...
    def collectProteinIDs(self):
        ...
        eSearch = Entrez.esearch(db='protein',term= proteinAccession, rettype='gb', api_key=self.apikey )
    ...
    def collectCDS(self):
        ...
        generalEfetch= SeqIO.read(Entrez.efetch(db='protein', id=proteinID, rettype='gb', retmode='text', api_key=self.apikey), 'genbank')
        ...
        recordEfetch = SeqIO.read(Entrez.efetch(db='nuccore', id=accession, seq_start=startseq, seq_top=endseq, rettype='fasta', api_key=self.apikey), 'fasta')
    ...
    def collectGenomicSeq(self):
        ...
        elinkResult = Entrez.read(Entrez.elink(db='gene', dbfrom='protein', id=proteinID, api_key=self.apikey))
        geneID = elinkResult[0]['LinkSetDb'][0]['Link'][0]['Id']

        generalEfetch= Entrez.read(Entrez.efetch(db='gene', id=geneID, rettype='fasta', retmode='xml', api_key=self.apikey), validate=False)
        ...
        genomeEfetch = SeqIO.read(Entrez.efetch(db='nuccore', id=accession, seq_start=int(startseq), seq_stop=int(endseq), rettype='fasta', api_key=self.apikey),'fasta')
    ...
    def collectProteinDomains(self):
        ...
        e_fetch = SeqIO.parse(Entrez.efetch(db='protein', id=proteinAccession, retmax=1000, rettype='gb', retmode='fasta',api_key=self.apikey), 'gb')
    ...
    def collectCommonName(self, id):
        ...
        try:
            ...
            taxonomy = Entrez.read(
                Entrez.efetch(db='taxonomy', id=tax_id, api_key=self.apikey))
...
class GenomicContext():
    def __init__(self, geneRecord, kbps, apikey):
    ...
    def fetchRecord(self):
        ...
        startseq = int(geneRecord.description.split(' ')[0].split(':')[1].split('-')[0]) - self.kbps

        #Gets the end sequence of the gene and moves 50k basepairs downstream
        endseq = int(geneRecord.description.split(' ')[0].split(':')[1].split('-')[0]) + self.kbps
                ...
                for gb_info in gb_gene['GBFeature_quals']:
                    ...
                    if gb_info['GBQualifier_name'] == 'db_xref':
                        try:
                            ...
                            pass
        GC_List = []
        GC = GenomicContext(self.geneRecord, apikey=self.apikey, kbps=self.kbps)
